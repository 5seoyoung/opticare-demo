...
const getDeviceIcon = (device) => {
  switch (device) {
    case "vent":
      return <Wind className="w-4 h-4" title="인공호흡기" />;
    case "crrt":
      return <Droplets className="w-4 h-4" title="CRRT" />;
    case "ecmo":
      return <Heart className="w-4 h-4" title="ECMO" />;
    default:
      return null;
  }
};

export default function DashboardView({ patients = [], onSelectPatient, beds = [], meds = [] }) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          ...
          <div className="text-2xl font-bold text-gray-900">
            {beds.filter((b) => !b.patientId).length} / {beds.length || 6}
          </div>
          <div className="text-xs text-gray-500 mt-1">
            점유율 {Math.round((1 - (beds.filter((b) => !b.patientId).length / (beds.length || 6))) * 100)}%
          </div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center justify-between mb-2">
      ...
      {/* 시뮬레이터 (대시보드 하위 섹션) */}
      <SimulationPanel patients={patients} allMeds={meds} />
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800">
                      {typeof p.cluster === "string" ? p.cluster : `C${p.cluster}`}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {p.alert && <AlertTriangle className="w-4 h-4 text-amber-600" />}
                  </td>
